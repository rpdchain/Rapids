...
enum MPRPCErrorCode
{
    ...
    MP_TX_IS_NOT_TOKEN_PROTOCOL    = -3336,  // No Assets Protocol transaction.
};
...
inline std::string error_str(int ec) {
  ...
  switch (ec)
  {
      ...
      case MP_INPUT_NOT_IN_RANGE:
          ec_str = "Input value supplied larger than supported in Assets Protocol";
          break;
      ...
      case PKT_ERROR_SP -25:
          ec_str = "Assets not issued yet";
          break;
      case PKT_ERROR_SP -26:
          ec_str = "Desired token must be Assets";
          break;
